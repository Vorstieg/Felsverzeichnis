			...
			loader.parse(
				...
				(gltf) => {
					...
					if (box.isEmpty()) {
						...
						console.log(
							`Calculated model position offset: [${offset.x.toFixed(2)}, ${offset.y.toFixed(2)}, ${offset.z.toFixed(2)}]`
						);
					}
				...
				if (parsed && typeof parsed === 'object') {
					...
					if (!userState.topo.routes) {
						...
						userState.topo.routes.forEach((r) => {
							if (!r.tags) r.tags = [];
							if (!r.id) r.id = generateRouteId();
							if (!r.fixPoints) r.fixPoints = [];
							// Ensure pitches also have IDs
							if (r.pitches) {
								r.pitches.forEach((p) => {
									if (!p.id) p.id = generateId('pitch');
								});
							}
						});
					}
					...
					if (!userState.topo.outlines) {
						...
						userState.topo.outlines.forEach((o) => {
							if (!o.id) o.id = generateOutlineId();
						});
					}

					// Re-initialize ID counters from new data
					initializeIdCounters(userState.topo);
				...
				topoToSave.routes = topoToSave.routes.map((route) => {
					...
					const processedPoints =
						route.points?.length > 0
							? route.points.map((p) => [p[0] + offset[0], p[1] + offset[1], p[2] + offset[2]])
							: [];
